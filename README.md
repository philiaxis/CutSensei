...
固定カメラで撮影した講義動画（黒板・ホワイトボード・電子黒板）や、GoodNotes などの電子ノートを使った
解説の画面録画を読み込み、音声と映像を解析して編集案を自動で作ります。
...
- **電子黒板・電子ノートの画面録画にも対応**：GoodNotes・Notability・OneNote・ホワイトボードアプリ・
  ペンで書き込むスライドなどの録画を自動で見分け、ペンの書き込みや消しゴムを正確に検出します。
  スクロールやページめくり、レーザーポインター、カーソル、時計の表示、講師のカメラ映像（ワイプ）は
  板書と誤認しません（[詳しく](#電子黒板電子ノートの画面録画)）。
...
同じ内容をタブレットのノートアプリで書いた画面録画でも、同じ結果になることを検証しています（`tests/test_screen.py`）。
...
**動画を読み込む → 必要に応じて板書範囲を指定 → 倍率などを設定 → 自動編集 → プレビューと修正 → 書き出し**

1. **読み込み**（Ctrl+I）で講義動画や画面録画を開きます。ウィンドウへのドラッグ＆ドロップでも開けます。
2. 必要なら **板書範囲** で黒板・ホワイトボード（画面録画ではノートのページ部分）を四角形で囲みます（複数可）。
   受講者やスクリーンの動きを板書と誤認しにくくなります。
...
![板書範囲の指定](docs/images/board_region_ja.png)

## 電子黒板・電子ノートの画面録画

iPad の GoodNotes・Notability、PC の OneNote・ホワイトボードアプリ・PDF への書き込みなどを画面録画した
解説動画にも対応しています。読み込むと **カメラ撮影か画面録画かを自動で判定** し（右パネル「素材の種類」に
判定結果を表示）、画面録画には専用の解析を使います。判定が違う場合は「素材の種類」で選び直してください。

![画面録画を解析した例](docs/images/main_screen_ja.png)

| 画面の変化 | 扱い |
|---|---|
| ペンで書く・消しゴムで消す・図形や文字を追加する | 板書（無言なら倍速）。書いた時刻を正確に記録 |
| スクロール・拡大縮小・ページめくり | 板書作業の一部として扱い、無言なら前後の板書とまとめて倍速。前のページの内容を「新しい板書」と数えない |
| レーザーポインター・カーソル・ペンのホバー・メニュー | 板書と見なさない（1.5 秒以上残らない変化や、すぐ元に戻る変化は除外）。無言でポインターを動かし続けている間は等速で残して確認対象に |
| ステータスバーの時計・電池表示など | 同じ場所で単独で繰り返す小さな変化として除外 |
| 講師のカメラ映像（ワイプ）・再生中の動画 | 常に動いている領域として自動で除外（板書範囲の画面にも表示） |
| 画面が止まっていて無言 | 待機として削除 |

- **板書範囲**：アプリのツールバーやステータスバーを除いたページ部分を指定すると、さらに確実になります（任意）。
  自動で除外したカメラ映像は、板書範囲の画面に斜線で表示されます。

  ![画面録画の板書範囲（カメラ映像は自動で除外）](docs/images/board_region_screen_ja.png)

- **音声**：iPad の画面収録ではコントロールセンターで **マイクをオン** にしてください。発話の判定は音声で行います。
- **可変フレームレート**：iPad の画面収録のように画面が止まっている間のフレームが省略された動画もそのまま扱えます。
- 教室の電子黒板（大型ディスプレイ）を **カメラで撮影した** 動画は、カメラ撮影として黒板と同じ方法で解析します。
...
```bash
# 解析して書き出しまで一度に（板書 4 倍、板書範囲を指定、NVENC で書き出し）
...
# GoodNotes などの画面録画（素材の種類は自動判定。--source screen / camera で指定も可）
cutsensei-cli auto goodnotes.mp4 --source screen

# 動作確認用の合成講義動画（説明60秒・板書40秒・待機20秒）を作る
cutsensei-cli demo demo_lecture.mp4
cutsensei-cli demo demo_notes.mp4 --scenario screen   # 同じ内容の電子ノートの画面録画
```
...
| 項目 | 説明 | 初期値 |
|---|---|---|
| 素材の種類 | 自動 / カメラ撮影（黒板・ホワイトボード）/ 画面録画（電子黒板・電子ノート） | 自動（動画ごとに判定） |
...
板書範囲、自動編集・書き出しの設定を保存します。素材を移動した場合は、プロジェクトと同じフォルダーにあれば
...
- **判定が合わない**：板書範囲を指定する、カットの積極性を下げる、詳細設定でしきい値を調整する、などを
  試してください。確認対象を順に見て、必要な区間は「必ず残す」で保護できます。
- **画面録画なのにカメラ撮影と判定される（またはその逆）**：右パネル「素材の種類」で正しい種類を選び、
  「自動編集を再適用」を押してください。写真や動画が画面の大部分を占める録画では自動判定が難しい場合があります。
